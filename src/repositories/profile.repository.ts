...
export class ProfileRepository {
  ...
  async getProfile({ publicIdentifier }: { publicIdentifier: string }): Promise<Profile> {
    ...
    const vectorImage = get(profile, 'profilePicture.displayImageReference.vectorImage');
    const pictureUrls = getProfilePictureUrls(vectorImage !== undefined ? vectorImage : undefined);
